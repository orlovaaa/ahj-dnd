...
export default class Tickets {
  ...
  init() {
    ...
    if (localStorage.getItem("cards"))
      this.ticketsArr = JSON.parse(localStorage.getItem("cards"));
  ...
  events() {
    ...
    this.tasks.addEventListener("pointermove", (e) =>
      this.ticketsMove.ticketMove(e)
    );
  ...
  static addAnotherTicketCancel() {
    ...
    form.remove();
    const addAnotherCardButtonHidden =
  ...
  deleteCard(e) {
    ...
    const deleteItemIndex = this.ticketsArr.findIndex(
  ...
  insertTicket(draggedEl) {
    ...
    const travelerTicket = this.ticketsArr.find(
      (ticket) => ticket.id === +draggedElRes.dataset.id
    );
    ...
    const travelerTicketIndex = this.ticketsArr.findIndex(
      (ticket) => ticket.id === +draggedElRes.dataset.id
    );
    ...
    if (!insertItemRes.id && !insertItemRes.position) {
      ...
      const insertTicketIndex = this.ticketsArr.findIndex(
        (ticket) => ticket.id === +insertItemRes.id
      );
      if (insertItemRes.position === "before")
        this.ticketsArr.splice(insertTicketIndex, 0, travelerTicket);
      if (insertItemRes.position === "after")
        this.ticketsArr.splice(insertTicketIndex + 1, 0, travelerTicket);
    }
  ...
  updateList() {
    ...
    this.ticketsArr.forEach((ticket) => {
      const col = Array.from(this.columnItems).find(
        (column) => ticket.column === column.dataset.id
      );
